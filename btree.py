...
class BTree:
    ...
    def add_record(self, index, a_probability, b_probability, sum_probability, depth_recurrency):
        if depth_recurrency == 0 and self._index_file.get_number_of_records() == 2 * self._d:
            # From btree.cpp
            ...
            self.add_record(index, a_probability, b_probability, sum_probability, depth_recurrency + 1)
        else:
            # from node.cpp
            i = self._index_file.get_number_of_records() - 1
            if self._index_file.get_number_of_sons() == 0:
                ...
                if ison.get_number_of_metadata_entries() == 2 * self._d - 1:
    ...
    def split_child(self, i, old_root):
        ...
        for j in range(0, self._d): # Edit ID1
            temp.metadata_entries[j] = old_root.metadata_entries[j + self._d - 1] # Edit ID1
        ...
        old_root.keys_count = self._d  # Edit ID1
        ...
        new_root.metadata_entries[i] = old_root.metadata_entries[self._d]  # Edit ID1
